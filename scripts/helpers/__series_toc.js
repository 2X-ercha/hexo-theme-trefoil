...
hexo.extend.helper.register('__series_toc', (page) => {
    const toc = hexo.extend.helper.get('toc').bind(hexo)
      ...
      for (const it of Object.values(series.pages)) {
        if (page.series_index == it.series_index) {
          toc_str += '<div class="doc-toc-tree-block active">'
            toc_str += '<a class="doc-toc-tree-h1" href='+url_for(it.path)+'>'
              toc_str += '<span class="toc-text">'+it.subtitle+'</span></a>'
            toc_str += toc(page.content)
          toc_str += '</div>'
        } else {
